...
class TalentoDAO
{
    ...
    {
        try {
            $sql = $this->conexao->prepare("UPDATE talentos SET nome = :nome, email = :email, fone = :fone, tecnologia = :tecnologia, senha = :senha, curriculo = :curriculo WHERE idTalento = :id");
            ...
            $sql->bindValue(':senha', $talento->getSenha());
            $sql->bindValue(':curriculo', $talento->getCurriculo());
    ...
    {
        try {
            $sql = $this->conexao->prepare("INSERT INTO talentos (nome, email, fone, tecnologia, senha, curriculo) VALUES (:nome, :email, :fone, :tecnologia, :senha, :curriculo)");
            $sql->bindValue(':nome', $talento->getNome());
            $sql->bindValue(':email', $talento->getEmail());
            $sql->bindValue(':fone', $talento->getFone());
            $sql->bindValue(':tecnologia', $talento->getTecnologia());
    ...
    {
        try {
            $sql = $this->conexao->prepare("SELECT * FROM talentos WHERE email = :email");
            ...
            if (password_verify($senha, $senhaHash)) {
                if ($dados) {
                    ...
                    error_log('Nenhum talento encontrado com os dados fornecidos.');
                    return null;
                }
            } else {
