...
class TelaJogo(Screen):
    ...
    def compose(self):
        yield Header()
        yield Static(f'Jogador: {Init.JOGADOR.nome}', id="stt_jogador")
        yield Static(f'Cena: {Init.CENA_ATUAL.nome}', id="stt_cena_atual")
    ...
    def criar_static_itens(self):
        ...
        for chave in Init.CENA_ATUAL.itens.keys():
            mensagem += f"[@click=pegar('{chave}')]Pegar[/]: {chave}\n"
        self.query_one("#stt_items_cena").update(mensagem)

    def action_pegar(self, item):
        Init.JOGADOR.inventario[item] = Init.CENA_ATUAL.itens[item]
        Init.CENA_ATUAL.pegar_item(item)
        self.on_screen_resume()

    def action_anda(self, direcao):
        match direcao:
            case "norte" if Init.CENA_ATUAL.norte:
                Init.CENA_ATUAL = Init.CENA_ATUAL.norte
            case "sul" if Init.CENA_ATUAL.sul:
                Init.CENA_ATUAL = Init.CENA_ATUAL.sul
            case "leste" if Init.CENA_ATUAL.leste:
                Init.CENA_ATUAL = Init.CENA_ATUAL.leste
            case "oeste" if Init.CENA_ATUAL.oeste:
                Init.CENA_ATUAL = Init.CENA_ATUAL.oeste
        self.on_screen_resume()
    ...
    def action_inventario(self):
        if self.inventario_aberto:
            ...
            self.mount(TextArea(f"".join(item for item in Init.JOGADOR.inventario.keys(
    ...
    def on_screen_resume(self):
        ...
        stt_jogador.update(f'Jogador: {Init.JOGADOR.nome}')
        stt_cena_atual.update(f'Cena: {Init.CENA_ATUAL.nome}')
